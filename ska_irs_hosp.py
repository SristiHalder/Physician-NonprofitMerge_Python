...
def match_ska_irs_hosp(ska_path, irs_paths, output_path, name_thresh=0.85, addr_thresh=0.95):
    print("Reading unmatched SKA hospital data...")
    ska = pd.read_csv(ska_path, usecols=["EXPL7", "ADDRESS1", "ZIP"], dtype=str, encoding="ISO-8859-1")
    ...
    for prefix in ska["zip_prefix"].dropna().unique():
        ...
        tfidf_ska_names = name_vectorizer.transform(ska_sub["EXPL7"].fillna(""))
...
#Run
match_ska_irs_hosp(
    ska_path="unmatched_code7_ska_total.csv",
    irs_paths=["eo1.csv", "eo2.csv", "eo3.csv"],
    output_path="matched_hosp_with_irs_name_and_address.csv",
    name_thresh=0.85,
    addr_thresh=0.95
)
